# Dental Clinic Lina - Dental Clinic Website

## Overview

Dental Clinic Lina is a multi-page dental clinic marketing website for a practice based in Istanbul, Turkey. It features bilingual support (English and Bulgarian), showcasing 9 dental services (implants, Hollywood smile, zirconium crowns, All-on-4, All-on-6, composite bonding, orthodontics, teeth whitening, porcelain veneers), doctor profiles, award display, certificates, VIP services, a Before & After gallery, contact information, and WhatsApp click tracking with analytics.
...
- Renamed to "Dental Clinic Lina" for SEO (matches "dental clinic" search terms)
- Enhanced service descriptions with detailed SEO content, added Composite Bonding as 9th service
- Added patient testimonials section (6 reviews from various countries)
- Added treatment comparison tables (All-on-4 vs All-on-6, Veneers vs Composite Bonding)
- Added FAQ accordion section (9 bilingual Q&As about treatments)
- Added Instagram section linking to @cliniclina with gallery grid
- Added interactive Before & After comparison slider component on gallery page
- WhatsApp clicks open in new tab from direct click (no popup blocker) then navigate to /thank-you
- Added WhatsApp click tracking: logs page, language, UTM params, referrer to database
- Admin dashboard at /admin/clicks for viewing WhatsApp click analytics (not linked publicly)
- Added Home link in navbar and footer for easy navigation
- Shared WhatsApp click handler utility at `client/src/lib/whatsapp-tracking.ts`
...
- `/admin/clicks` — WhatsApp click analytics dashboard (not linked publicly)

### Key Components
- `client/src/components/Navbar.tsx` — Responsive navbar with language switching, WhatsApp button, Home link
- `client/src/components/Footer.tsx` — Site footer with links, contact info, Home link
...
- `client/src/lib/whatsapp-tracking.ts` — Shared WhatsApp click handler (opens WhatsApp, tracks to DB, redirects to thank-you)
